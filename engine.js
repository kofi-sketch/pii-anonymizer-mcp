...
const patterns = [
  ...
  { id:"phone_contextual", label:"Phone (contextual)", tier:"Contact", pattern:/\b(?:tel|phone|ph|mobile|mob|cell|fax|contact|call|number|no)\s*[:\-#]?\s*(\(?\+?[\d][\d\s.\-()]{6,19}[\d)])/gi, validate:(m)=>{const d=m.replace(/\D/g,"");if(d.length<7||d.length>15)return false;const hasFormatting=/[\+\-().]/.test(m)||/\d\s+\d/.test(m);const hasStrongKeyword=/\b(?:tel|phone|ph|mobile|mob|cell|fax|contact|call)\b/i.test(m);const hasWeakKeyword=/\b(?:number|no)\b/i.test(m);if(!hasFormatting&&hasWeakKeyword&&!hasStrongKeyword)return false;return true;}, placeholder:"PHONE_NUMBER", captureGroup:1 },
