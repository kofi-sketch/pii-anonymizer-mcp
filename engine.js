...
const patterns = [
  ...
  { id:"api_key_secret", label:"API Key / Secret", tier:"Crypto", pattern:/\b(?:sk_live_|sk_test_|pk_live_|pk_test_|sk-[a-zA-Z0-9]{20,}|ghp_[a-zA-Z0-9]{36,}|github_pat_[a-zA-Z0-9_]{20,}|glpat-[a-zA-Z0-9\-]{20,}|xox[bpsa]-[a-zA-Z0-9\-]{10,})[a-zA-Z0-9_\-]{10,}\b/gi, placeholder:"API_KEY" },
  { id:"credit_card", label:"Credit Card", tier:"Financial", pattern:/\b(?:4[0-9]{3}|5[1-5][0-9]{2}|3[47][0-9]{2}|6(?:011|5[0-9]{2}))[- ]?[0-9]{4}[- ]?[0-9]{4}[- ]?[0-9]{1,4}\b/g, validate:(m)=>luhnCheck(m), placeholder:"CREDIT_CARD" },
  ...
  { id:"phone_intl", label:"Phone (International)", tier:"Contact", pattern:/\+[1-9]\d{0,3}[\s.\-]?\(?\d{1,4}\)?[\s.\-]?\d{1,4}[\s.\-]?\d{1,9}/g, validate:(m)=>{const d=m.replace(/\D/g,"");return d.length>=7&&d.length<=15;}, placeholder:"PHONE_NUMBER" },
  ...
  { id:"uk_postcode", label:"UK Postcode", tier:"Address", pattern:/\b[A-Z]{1,2}\d[A-Z\d]?\s*\d[A-Z]{2}\b/gi, placeholder:"ZIP_CODE" },
  { id:"us_zip", label:"US ZIP Code", tier:"Address", pattern:/\b\d{5}(?:-\d{4})?\b/g, placeholder:"ZIP_CODE" },
  ...
  { id:"date_full", label:"Date", tier:"Contact", pattern:/\b(?:\d{1,2}[\/-]\d{1,2}[\/-]\d{2,4}|\d{4}-\d{2}-\d{2})\b/g, placeholder:"DATE" },
  ...
  { id:"name_dictionary", label:"Known Name", tier:"Names", pattern:/\b[a-zA-Z]{2,}\b/g, validate:(m,t,i)=>{const lo=m.toLowerCase();if(SUPPRESS_WORDS.has(lo))return false;if(AMBIGUOUS_NAME_WORDS.has(lo)){const before=t.slice(Math.max(0,i-60),i);if(!NAME_CONTEXT_BEFORE.test(before))return false;}return FIRST_NAMES.has(lo)||LAST_NAMES.has(lo);}, placeholder:"PERSON_NAME" },
  { id:"device_id", label:"Device ID", tier:"System", pattern:/\b(?:DEV|DEVICE|device)[_\-][a-zA-Z0-9_\-]{6,}\b/g, placeholder:"DEVICE_ID" },
  { id:"session_id", label:"Session Token", tier:"System", pattern:/\b(?:SESSION|session|sess|TOKEN|token)[_\-][a-zA-Z0-9_\-]{6,}\b/g, placeholder:"SESSION_TOKEN" },
