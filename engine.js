...
const patterns = [
  ...
  { id:"us_zip", label:"US ZIP Code", tier:"Address", pattern:/\b\d{5}(?:-\d{4})?\b/g, validate:(m,t,i)=>{const before=t.slice(Math.max(0,i-30),i);if(/[":]\s*$/.test(before)&&/[{,]/.test(before))return false;if(/,\s*$/.test(before)){const lineStart=t.lastIndexOf("\n",i-1);const line=t.slice(lineStart+1,i+m.length+20);if(/^\s*\w+,/.test(line))return false;}const after=t.slice(i+m.length,i+m.length+5);if(/^\s*[,}]/.test(after)&&/[{"]/.test(before))return false;return true;}, placeholder:"ZIP_CODE" },
  ...
  { id:"name_dictionary", label:"Known Name", tier:"Names", pattern:/\b[a-zA-Z]{2,}\b/g, validate:(m,t,i)=>{const lo=m.toLowerCase();if(SUPPRESS_WORDS.has(lo))return false;if(AMBIGUOUS_NAME_WORDS.has(lo)){const before=t.slice(Math.max(0,i-60),i);if(NAME_CONTEXT_BEFORE.test(before))return true;const immBefore=t.slice(Math.max(0,i-30),i);const prevWordMatch=immBefore.match(/\b([A-Z][a-z]+)\s+$/);if(prevWordMatch&&FIRST_NAMES.has(prevWordMatch[1].toLowerCase()))return true;return false;}return FIRST_NAMES.has(lo)||LAST_NAMES.has(lo);}, placeholder:"PERSON_NAME" },
